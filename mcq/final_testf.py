...
# Constants for proctoring
ACTIVITY_CHECK_INTERVAL = 3  # Interval in seconds for activity monitoring
MAX_INACTIVITY_SECONDS = 10   # Maximum allowed inactivity in seconds
MAX_PROCESS_SWITCHES = 3      # Maximum allowed switches to other processes
...
# AI Proctoring Middleware
def ai_proctor_middleware():
    global MAX_INACTIVITY_SECONDS, MAX_PROCESS_SWITCHES
    current_process = psutil.Process(os.getpid())
    process_switch_count = 0
    last_active_time = time.time()

    def monitor_activity():
        nonlocal process_switch_count, last_active_time
        while True:
            current_active_window = None  # Implement fetching active window/process
            if current_active_window != "Technical Test":  # Replace with your app's window title
                process_switch_count += 1
                last_active_time = time.time()

            if process_switch_count > MAX_PROCESS_SWITCHES:
                # Redirect to finish page due to unauthorized process switches
                return redirect(url_for('finish'))

            if time.time() - last_active_time > MAX_INACTIVITY_SECONDS:
                # Redirect to finish page due to inactivity
                return redirect(url_for('finish'))
...
if __name__ == '__main__':
    # Start AI proctoring middleware
    ai_proctor_middleware()
